...
\begin{figure}[h!]
	...
	\end{tabular}
	\caption{\small Simulation parameters estimated from consumption curves: (a) estimated local exponent function, (b) estimated mean function, (c) estimated autoregressive kernel. \color{magenta}[TBC: panel (a) shows an estimated exponent; the design now uses an analytic linear ramp]\color{black}}
...
\color{magenta}
We conclude this section with a performance comparison of the new autocovariance estimator against the \texttt{MPV24} estimator, which uses a single bandwidth. Let $\widehat{c}_{N,\ell}^{(i)}$ denote our new estimator computed from the $i$-th replication ($i = 1, \ldots, R = 400$), and let $\widehat{c}_{\texttt{MPV24}, \ell}^{(i)}$ denote the corresponding \texttt{MPV24} estimator. For benchmarking, we use the empirical infeasible estimator, denoted by ${\widehat{c}}_{N', \ell}$, which is computed on a large sample of size $N'$. This estimator is feasible in simulation settings. That is,
\color{black}
\begin{equation}
...
\begin{equation}\label{eq:error_ratio_autocov}
	...
	E_\ell^{(i)}(s, t) = \frac{\left| \widehat{c}_{N,\ell}^{(i)}(s, t; h_\ell^*(s|t), h_\ell^*(t|s)) - \widehat{c}_{N',\ell}(s, t) \right|}{\left| \widehat{c}_{\texttt{MPV24}, \ell}^{(i)}(s, t; h^*) - \widehat{c}_{N',\ell}(s, t) \right|}.\color{black}
\end{equation}
\color{magenta}
[TBC: the infeasible benchmark protocol must be restated from 11\_simulation\_generate.R:214--227 --- $N' = 5000$, common design, noiseless, on the grid \texttt{seq(0.01, 0.99, len = 99)}]
\color{black}

\color{magenta}
[TBC: lag-1 error-ratio results. The source paragraph quotes $(H_{0.2}, H_{0.3}) = (0.199, 0.844)$, $(H_{0.5}, H_{0.2}) = (0.784, 0.199)$, $(H_{0.1}, H_{0.4}) = (0.529, 0.527)$ and $(H_{0.8}, H_{0.5}) = (0.445, 0.784)$; these come from the superseded estimated Hurst function and do not hold under \texttt{hurst\_linear(h\_left = 0.4, h\_right = 0.8)}. Results require 20\_simulation\_autocov.R then 30\_figures\_autocov.R, neither of which has been run: data-simulation/estimates/ has no autocov/ directory]
\color{black}

\color{magenta}
[TBC: lag-0 error-ratio results and the accompanying boxplot figure. The source compares the covariance estimator against its own single-bandwidth version rather than against \texttt{MPV24}. Requires 21\_simulation\_cov.R then 31\_figures\_cov.R; data-simulation/estimates/ has no cov/ directory]
\color{black}
